...
st.header("Cherry: Chatting with Kotak Cherry")

# Initialize embeddings
embeddings = OpenAIEmbeddings()

# Load the Buffett and Branson databases
LocalDB = Chroma(persist_directory=os.path.join('db', 'buffett'), embedding_function=embeddings)
...
def Local_handler(query):
    ...
    if not relevant_docs:
        print("Data not available in Buffett's database. Fetching answer from ChatGPT...")
        ...
        return {"role": "bot", "content": answer}

    print("Data fetched from Buffett's database.")
...
def chatgpt_fetch_answer(query):
    ...
    response = openai.Completion.create(
        ...
        prompt=query,
        custom_prompt = "We don't provide any financial advice!",
        max_tokens=50  # Adjust the max_tokens as needed
    )
    answer = response.choices[0].text.strip()
    return answer

def Local_handler2(custom_prompt):
    try:
        print("working on advisory query", custom_prompt)
        # Process the query or perform some actions related to the query
        
        # Assuming you want to use the ChatGPT API to fetch an answer based on the query
        # answer = chatgpt_fetch_answer(query)
        no_advice_response = "At Kotak Cherry, we do not offer financial advice."
        return {"role": "system", "content": custom_prompt}
    
    except Exception as e:
        # Return a statement indicating that Kotak Cherry does not provide financial advice
        no_advice_response = "At Kotak Cherry, we do not offer financial advice."
    
        return {"role": "user", "content": custom_prompt}  # Strip whitespace from the content


def other_handler(custom_prompt):
    print("working on advisory query with one parameter", custom_prompt)
...
def route_by_category(query, category):
    if category == "0":
        ...
        custom = "We don't provide financial advice."
        return Local_handler2(custom)
    elif category == "3":
        custom = "We don't provide financial advice."
        return other_handler(custom)
    # else:
